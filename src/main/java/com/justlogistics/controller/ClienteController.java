...
public class ClienteController {
	...
	@PostMapping
	public ResponseEntity<ApiResponseJust<?>> guardar(@Valid @RequestBody Cliente request,
		...
		try {
			...
			return ResponseEntity
					.ok(new ApiResponseJust<>("Cliente creado correctamente.", HttpStatus.OK.value(), creado));
	...
	public ResponseEntity<ApiResponseJust<?>> listar() {
		try {
			...
			return ResponseEntity
					.ok(new ApiResponseJust<>("Listado obtenido correctamente.", HttpStatus.OK.value(), clientes));
	...
	public ResponseEntity<ApiResponseJust<?>> get(@PathVariable Integer id) {
		try {
			...
			return ResponseEntity.ok(new ApiResponseJust<>("Cliente encontrado.", HttpStatus.OK.value(), cliente));
	...
	@PutMapping("/{id}")
	public ResponseEntity<ApiResponseJust<?>> actualizar(@PathVariable Integer id, @Valid @RequestBody Cliente request,
			BindingResult bindingResult) {
		try {
			if (bindingResult.hasErrors()) {
				String errorMsg = bindingResult.getFieldError().getDefaultMessage();
			...
			return ResponseEntity.ok(new ApiResponseJust<>("Cliente actualizado.", HttpStatus.OK.value(), actualizado));
	...
	public ResponseEntity<ApiResponseJust<?>> eliminar(@PathVariable Integer id) {
		try {
			...
			if (eliminado) {
				return ResponseEntity.ok(new ApiResponseJust<>("Cliente eliminado.", HttpStatus.OK.value(), null));
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponseJust<>(
						"Cliente no encontrado para eliminar.", HttpStatus.NOT_FOUND.value(), null));
			}
